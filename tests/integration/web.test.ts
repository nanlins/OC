/**
 * web.test.ts ?”â€?Web ç®¡ç??§åˆ¶?°æ?è¯•ï?REST ?•å½± + å®¡æ‰¹?¨ä? + SSEï¼? *
 * ä¿®æ”¹è®°å?ï¼? *   2026-08-13 ?›å»ºï¼ˆé˜¶æ®?9ï¼? */
...
let port: number;
// fix-plan P0ï¼šWEB_TOKEN fail-closedï¼Œæ?è¯•ç? vitest.config æ³¨å…¥ test-web-token
...
describe("web api", () => {
  ...
  it("static frontend served at /", async () => {
    ...
    expect(html).toContain("OC ç®¡ç??§åˆ¶??);
  });
  ...
  it("build:web ??React dist preferred and hashed assets served (fix-plan P2 regression)", async () => {
    ...
    if (!existsSync(distIndex)) return; // ?ªæ?å»ºå?è·³è?ï¼ˆCI ??build:web ?æ?ï¼?    // ?™æ€æ ¹åº”æ???React dist
    expect(resolveStaticDir()).toBe(resolvePath(PROJECT_ROOT, "web", "frontend", "dist"));
    // / åº”ä¸º React ?¥å£
    const home = await fetch(`http://127.0.0.1:${port}/`);
    expect(home.status).toBe(200);
    // dist/assets ä¸‹ç? .js åº”å¯?åŠ¡ä¸?MIME æ­?¡®
  ...
  it("traces endpoint rejects path traversal (fix-plan P0 regression)", async () => {
    // ç¼–ç??ç? ..%2F è§??ä¸?../ï¼Œå?é¡»è¢«?’ï?400ï¼‰ï?ä¸å?è¯»å? traces ?®å?ä¹‹å?
    ...
    // ?ˆæ? idï¼ˆä?å­˜åœ¨ï¼‰å?è¿”å? 200 + ç©ºæ•°ç»?    const ok = await fetch(`http://127.0.0.1:${port}/api/traces/some-valid-session`, { headers: AUTH });
  ...
  it("oversized POST body returns 413 (fix-plan P1 regression)", async () => {
    const big = "x".repeat(1024 * 1024 + 1024); // > 1MB ä¸Šé?
    const res = await fetch(`http://127.0.0.1:${port}/api/wirings`, {
      method: "POST",
      headers: { "content-type": "application/json", ...AUTH },
  ...
  it("stopWebServer terminates SSE connections (P1-3 regression)", async () => {
    ...
    expect(res.status).toBe(200);
    await stopWebServer();
    // è¿æ¥åº”è¢«?åŠ¡ç«¯ç?æ­¢ï?done ??reject(terminated) ?‡ä¸ºç»ˆæ­¢å½¢æ€?    const reader = res.body?.getReader();
    ...
    try {
      ...
      terminated = true; // other side closed = ?åŠ¡ç«¯å·²ç»ˆæ­¢è¿æ¥
    }
    expect(terminated).toBe(true);
    controller.abort();
    // ?å¯ä¾?afterEach stop å¹‚ç?
    port = await startWebServer(0);
  });
});
