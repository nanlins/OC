/**
 * feishu.test.ts ?”â€?é£ä¹¦?‚é??¨é??æ?è¯•ï?çº¯æœ¬??mockï¼Œä?è¿å?ç½‘ï?
 *
 * ?Œè´£ï¼šdeliver ?ˆå? tenant_access_token ?å? im/v1/messagesï¼ˆURL/?´æ?/body/è¿”å? message_idï¼‰ï?
 *       token ç¼“å?ä¸é?å¤è¯·æ±‚ï??­æ®ç¼ºå¤± factory è¿”å? nullï¼? *       parseFeishuEvent ä¸‰å½¢?ï?ç¾?mention/ç¾¤æ? mention/?•è?ï¼? ?ç›®?‡ä?ä»¶è???null?? * ä¿®æ”¹è®°å?ï¼? *   2026-08-13 ?›å»ºï¼ˆé˜¶æ®?10ï¼? */
...
describe("feishu adapter", () => {
  ...
  it("caches tenant_access_token across delivers", async () => {
    ...
    expect(calls).toHaveLength(3); // ç¬¬ä?æ¬¡æ??’ä??å? token
  ...
  it("parseFeishuEvent parses group message with mention", () => {
    const parsed = parseFeishuEvent(
      receiveEvent("group", [{ key: "@_user_1", name: "å°åŠ©??, id: { open_id: "ou_bot", app_id: "cli_a1" } }]),
    );
