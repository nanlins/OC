/**
 * slack.test.ts ?î‚Ä?Slack ?ÇÈ??®È??êÊ?ËØïÔ?Á∫ØÊú¨??mockÔºå‰?ËøûÂ?ÁΩëÔ?
 *
 * ?åË¥£Ôºödeliver chat.postMessage ??URL/?¥Ê?/body/ËøîÂ? tsÔºõÂá≠?ÆÁº∫Â§?factory ËøîÂ? nullÔº? *       hello ?°Ê? + events_api ack + ?•Á?‰∏âÂΩ¢?ÅÔ?Áæ??ïË?/mentionÔºâÔ??≠Á∫ø?áÊï∞?Ä?øÈ?Ëøû„Ä? * ‰øÆÊîπËÆ∞Â?Ôº? *   2026-08-13 ?õÂª∫ÔºàÈò∂ÊÆ?10Ôº? */
...
describe("slack adapter", () => {
  ...
  it("reconnects with exponential backoff after socket close", async () => {
    ...
    try {
      ...
      expect(sockets).toHaveLength(1); // È¶ñÊ¨°?Ä??1000ms ?™Âà∞
      await vi.advanceTimersByTimeAsync(1);
      expect(sockets).toHaveLength(2);
      sockets[1]!.onclose!();
      await vi.advanceTimersByTimeAsync(1999);
      expect(sockets).toHaveLength(2); // ‰∫åÊ¨°?Ä?øÁøª?çËá≥ 2000ms
      await vi.advanceTimersByTimeAsync(1);
