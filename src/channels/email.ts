/**
 * channels/email.ts ?î‚Ä?Email ?öÈ??ÇÈ??®Ô?IMAP ËΩÆËØ¢?•Á? + SMTP ?∫Á?Ôº? *
 * ?åË¥£ÔºöÊ?Â∞?IMAP ÂÆ¢Êà∑Á´ØÔ?CAPABILITY/LOGIN/SELECT INBOX/UID SEARCH/UID FETCH/UID STORE Â≠êÈ?ÔºåË??èËÆÆ + {N} literalÔºâÔ?
 *       ËΩÆËØ¢ UNSEEN ??onInboundÔºàplatformId=email:<user>?ÅsenderId=email:<from>?ÅisMention=trueÔºâÔ?
 *       ?ÄÂ∞?SMTP ÂÆ¢Êà∑Á´ØÔ?EHLO/AUTH LOGIN/MAIL FROM/RCPT TO/DATAÔºå\r\n.\r\n ÁªàÊ≠¢ÔºâÔ?socketFactory Ê≥®ÂÖ•?ØÊ??? * ?≥ÈîÆÂØºÂá∫ÔºöcreateEmailAdapter, registerEmailChannel, MailSocket, SocketFactory
 * ?øÈ?‰∏çÂ??èÔ?UID ?ïË??®Ë??ªÈ?ÔºàÂ?‰∏Ä UNSEEN ?óË°®‰∫åÊ¨°ËΩÆËØ¢‰∏çÈ?Â§çÂÖ•Á´ôÔ?ÔºõSMTP DATA ÂøÖÈ°ª‰ª?\r\n.\r\n ÁªìÊ??? * ?üÈâ¥Ôºönanoclaw channels ?ÜÊîØ email ÂΩ¢ÊÄ? *
 * ‰øÆÊîπËÆ∞Â?Ôº? *   2026-08-13 ?õÂª∫ÔºàÈò∂ÊÆ?10Ôº? */
...
export interface MailSocket {
  ...
  /** fix-plan P1ÔºöÂ?Â±?socketÔºà‰? STARTTLS ?áÁ∫ß?ÖË£πÔºâÔ?ÊµãË???socket ?ØÊ?Ê≠§Â?ÊÆ?*/
  raw?: unknown;
}
...
/**
 * fix-plan P1ÔºàSTARTTLSÔºâÔ??®Â∑≤Âª∫Á??ÑÊ???socket ‰∏äÂ?Á∫?TLSÔºàÈ?ËØÅË?‰π¶Ô?rejectUnauthorized ÈªòËÆ§ trueÔºâ„Ä? * ?áÁ∫ß?êÂ?ËøîÂ??ÖË£π?éÁ? TLS MailSocketÔºõÂ?Â±?socket Áº∫Â§±?ñÊè°?ãÂ§±Ë¥•Â? rejectÔºàË??®Êñπ?íÁ?ËÆ§Ë?ÔºåÂá≠?Æ‰?Ëµ∞Ê??áÔ??? */
...
export function createEmailAdapter(deps: EmailDeps): ChannelAdapter & { pollOnce: () => Promise<void> } {
  ...
  async function deliver(
    ...
    // fix-plan P1Ôº?65 ?êÂ? TLSÔºõÂÖ∂‰ΩôÁ´Ø???Â¶?587ÔºâÊ??áË??•Â?ÂøÖÈ°ª STARTTLS ?áÁ∫ßÔºåÂá≠?ÆÂè™??TLS ‰∏äÂ???    const implicitTls = smtpPort === 465;
    ...
    try {
      ...
      if (!implicitTls) {
        ...
        // ?áÁ∫ß TLSÔºàÈ?ËØÅË?‰π¶Ô?ÔºõÂ§±Ë¥•Âç≥?õÈ?ÔºåÁ?‰∏çÈÄÄ?ûÊ??áËÆ§ËØ?        const tlsSocket = await upgradeToTls(socket, smtpHost);
        socket = tlsSocket;
